...
fn place_objects(room: Rect, map: &Map ,objects: &mut Vec<Object>) {
    ...
    for _ in 0..num_monsters {
        let x = rand::thread_rng().gen_range(room.x1, room.x2);
...
fn handle_keys(tcod: &mut Tcod, game: &Game, objecs: &mut [Object]) -> bool {
    let key = tcod.root.wait_for_keypress(true);
    match key {
        Key { code: Up, ..} => move_by(PLAYER, 0, -1, &game.map, objecs),
        Key { code: Down, ..} => move_by(PLAYER, 0, 1, &game.map, objecs),
        Key { code: Left, ..} => move_by(PLAYER, -1, 0, &game.map, objecs),
        Key { code: Right, ..} => move_by(PLAYER, 1, 0, &game.map, objecs),
        Key { code: Escape, ..} => {
            return true;
        }
        _ => {}
...
fn main() {
    ...
    let player = Object::new(25,23, '@', "player", WHITE, true);
